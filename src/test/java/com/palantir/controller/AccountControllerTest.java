...
public class AccountControllerTest {
    ...
    public void alarmTest() throws Exception {
        when(accountService.alarmList(any(), any())).thenReturn(Page.empty());
        mockMvc.perform(get("/api/v1/account/alarm")
                .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk());
    }
    ...
    @Test
    @WithAnonymousUser
    public void alarmFailedCuzUnAuthorizedTest() throws Exception {
        when(accountService.alarmList(any(), any())).thenReturn(Page.empty());
        mockMvc.perform(get("/api/v1/account/alarm")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
